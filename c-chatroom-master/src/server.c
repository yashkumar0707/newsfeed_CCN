...
void send_to_all_clients(ClientList *np, char tmp_buffer[]) {
    ...
    while (tmp != NULL) {
        int n=strlen(np->name);
        int n1=strlen(tmp->name);
        if (np->data != tmp->data && (np->name)[n-1]==(tmp->name)[n1-1]) { // all clients except itself.
...
void client_handler(void *p_client) {
    ...
    if (recv(np->data, nickname, LENGTH_NAME, 0) <= 0 || strlen(nickname) < 2 || strlen(nickname) >= LENGTH_NAME-1) {
        ...
        printf("%s(%s)(%d) joined the news feed.\n", np->name, np->ip, np->data);
        sprintf(send_buffer, "%s(%s) joined the news feed.", np->name, np->ip);
